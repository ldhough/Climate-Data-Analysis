...
spark_location='/Users/lanniehough/Library/Python/2.7/lib/python/site-packages/pyspark'
...
if len(sys.argv) == 3: #.py COUNTRYCODE SINGLEYEAR
    ...
    if int(sys.argv[2]) > 1899 and int(sys.argv[2]) < 2019: #latest start year will be 2018
...
if len(sys.argv) == 4: #.py COUNTRYCODE STARTYEAR ENDYEAR
    ...
    if foundCountry == False:
        print("Invalid country code.")
        sys.exit(2)
    if int(sys.argv[2]) > 1899 and int(sys.argv[2]) < 2019:
        startYear = int(sys.argv[2])
        print("You indicated start year ", startYear)
        startTrue = True
    if startTrue == False:
...
pathToJson = './TempData2.json'
dataFrame = spark.read.json(pathToJson, multiLine=False).repartition(200)
...
def calcAvgChangeForMonth(m):
    ...
    year = 0
    while year != endYear-startYear: #arr starts at 0, years start at 1900 and go to 2020/2019
